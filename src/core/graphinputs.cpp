...
namespace evoplex {
        ...
        const QStringList& values, QString& errMsg)
    {
        ...
        checkAttrCommands(gi.get(), failedAttrs);
    ...
    void GraphInputs::checkAttrCommands(GraphInputs* gi, QStringList& failedAttrs)
    {
        auto gAttrs = gi->m_generalAttrs;
        /*
        // node command
        ...
        if (!QFileInfo::exists(nodeCmd)) { // not an existing file
            QString error;
            auto nAttrs = AttrsGenerator::parse(gi->m_modelNodeAttrScope, nodeCmd, error);
            if (!nAttrs || !error.isEmpty()) {
                failedAttrs.append(GENERAL_ATTR_NODES);
            }
        }
            
        if (gi->graphPlugin()->supportsEdgeAttrsGen()) {
            // if the graph supports edge commands, make sure it's valid
            QString error;
            auto eAttrs = AttrsGenerator::parse(gi->m_modelEdgeAttrScope,
